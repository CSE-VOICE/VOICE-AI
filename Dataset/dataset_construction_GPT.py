...
from langchain_openai import ChatOpenAI  # OpenAI용으로 변경
...
def create_routine_chain(llm: ChatOpenAI):
...
def generate_routines(num_routines=10000, excel_path='smart_home_routines.xlsx'):
    ...
    while len(all_routines) < num_routines and attempts < max_attempts:
        try:
            result = chain.invoke({"input_text": "퇴근하고 집에 왔는데 너무 더워."})
            print(result)  # 디버깅 출력
